...
export default function QuestionRadioSection() {
  ...
  const removeOption = (qid: number, index: number) => {
    setQuestions((prev) =>
      prev.map((q) => {
        ...
        const isCorrectAnswer = q.correctAnswer === removedValue;

        return {
          ...q,
          options: q.options.filter((_, i) => i !== index),
          correctAnswer: isCorrectAnswer ? '' : q.correctAnswer,
        };
      })
    );
  };
        ...
        {questions.map((q, idx) => (
          <div key={q.id} className="rounded-lg border border-gray-200 bg-gray-50 p-4">
            ...
            <div className="mt-4 flex flex-col gap-2">
              <label className="text-sm font-medium text-muted-foreground">Success Message:</label>
              <Input
